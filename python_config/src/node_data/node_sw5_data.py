...
def sw5_Structures(topology: Topology):
	...
	node_sw5=Node(
		...
		additional_config=sw5_additional_config
	)
...
def sw5_relations(topology: Topology):
	...
	sw5_node.get_interface("ethernet","0/0").connect_to(sw3_node.get_interface("ethernet","0/3"))
	sw5_node.get_interface("ethernet","0/1").connect_to(sw4_node.get_interface("ethernet","1/2"))
	sw5_node.get_interface("ethernet","3/3").connect_to(topology.get_exit_interface('exit_oob'))

def sw5_additional_config(node: Node):
	if node.hostname != 'sw5':
